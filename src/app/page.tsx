...
const socials = [
  ...
  },
  {
    ...
    color: "radial-gradient(circle at 30% 110%, #fdf497 0%, #fdf497 5%, #fd5949 45%, #d6249f 60%, #285AEB 90%)",
    icon: (
      <svg viewBox="0 0 24 24" className="h-4 w-4" fill="#ffffff">
  ...
  {
    label: "WhatsApp",
    href: "https://wa.me/254795853879?text=" + encodeURIComponent("Hi Archstruc Group, I'm interested in your services."),
    color: "#25D366",
    icon: (
      <svg viewBox="0 0 24 24" className="h-4 w-4" fill="#ffffff">
...
function CustomCursor() {
  ...
  return (
    <div
      ref={cursorRef}
      className={`pointer-events-none fixed top-0 left-0 z-[999] flex h-20 w-20 items-center justify-center rounded-full border border-white/40 backdrop-blur-sm transition-[opacity,transform] duration-200 ${
        hovering ? "scale-100 opacity-100 bg-[#358CB8]/20" : "scale-0 opacity-0"
      }`}
    >
...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen text-white">
      ...
      {/* ── Social icons ─────────────────────────────────────────────────── */}
      <div className="fixed bottom-6 right-4 z-[60] flex flex-col items-center gap-3 sm:right-6">
        {socials.map((s) => (
          <Link
            key={s.label}
            href={s.href}
        ...
        <div className="flex h-20 w-full items-center justify-between px-6 lg:px-10">
          <Link href="/" className="flex items-center gap-3" onClick={() => setMobileOpen(false)}>
            ...
            <span className="flex flex-col leading-none">
              <span className="text-xl font-semibold tracking-tight">ARCHSTRUC</span>
              <span className="text-xs uppercase tracking-[0.3em] text-[#358CB8]">Group</span>
            </span>
          </Link>
      ...
      <section className="relative h-screen w-full overflow-hidden">
        ...
        <div className="pointer-events-none absolute bottom-24 left-6 z-20 lg:left-10">
          <p
            key={projectIndex}
            className={`text-lg font-light tracking-wide text-white md:text-xl ${titleVisible ? "title-in" : "opacity-0"}`}
          >
